...
export default function ServiceModal({ isOpen, onClose, service }: ServiceModalProps) {
  ...
  const handleRequestService = () => {
    if (isAuthenticated) {
      // Redirect to dashboard with service pre-filled
      router.push(`/dashboard/customer/services?service=${encodeURIComponent(service.title)}`);
      onClose();
    } else {
        ...
        <div className="p-8">
          ...
          <div className={`bg-gradient-to-br ${service.gradient} rounded-xl p-6 text-white`}>
            ...
            <p className="mb-4 opacity-90">
              {isAuthenticated 
                ? `Request this service now and we'll get back to you with a quote.`
                : `Create an account or sign in to request this service, or continue as a guest.`
              }
            </p>
            ...
            {!showGuestOption ? (
              ...
              <div className="space-y-3">
                ...
                <Link href={`/register?service=${encodeURIComponent(service.title)}`} className="block">
                  <Button 
                    size="lg" 
                ...
                <Link href={`/login?service=${encodeURIComponent(service.title)}`} className="block">
                  <Button 
                    size="lg" 
