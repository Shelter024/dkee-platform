...
export default function DashboardLayout({
  ...
  useEffect(() => {
    const stored = localStorage.getItem('dk_customer_sidebar_collapsed');
    if (stored === 'true') {
  ...
  const toggleCollapse = () => {
    ...
    setSidebarCollapsed((prev) => {
      const next = !prev;
      localStorage.setItem('dk_customer_sidebar_collapsed', String(next));
      return next;
    });
  };
  ...
  const customerLinks = [
    ...
    { href: '/dashboard/customer/services', label: 'My Services', icon: Car },
    { href: '/dashboard/customer/vehicles', label: 'My Vehicles', icon: Car },
    { href: '/dashboard/customer/service-reminders', label: 'Service Reminders', icon: Bell, premium: true },
    { href: '/dashboard/customer/tracking', label: 'Vehicle Tracking', icon: Navigation, premium: true },
    { href: '/dashboard/customer/properties', label: 'Properties', icon: Building2 },
    { href: '/dashboard/customer/account', label: 'Account Settings', icon: User },
    { href: '/dashboard/customer/emergency', label: 'Emergency', icon: AlertCircle },
        ...
        <div className="flex flex-col h-full">
          ...
          <div className={`flex items-center ${sidebarCollapsed ? 'justify-center' : 'justify-between'} h-16 px-4 border-b transition-all duration-300`}>
            <Link href="/" className={`flex items-center ${sidebarCollapsed ? 'space-x-0' : 'space-x-2'}`}>
              ...
              {!sidebarCollapsed && <span className="font-bold text-gray-900 whitespace-nowrap">Dashboard</span>}
            </Link>
            {!sidebarCollapsed && (
              <button
                onClick={() => setSidebarOpen(false)}
              ...
              {customerLinks.map((link) => {
                ...
                return (
                  <Link
                    ...
                    href={link.href}
                    onClick={() => setSidebarOpen(false)}
                    className={`group relative flex items-center ${sidebarCollapsed ? 'justify-center px-2' : 'space-x-3 px-3'} py-2 rounded-lg transition-colors ${
                      isActive(link.href)
        ...
        <header className="bg-white shadow-sm h-16 flex items-center px-4 lg:px-6 relative">
          ...
          <div className="flex-1">
            <h1 className="text-xl font-semibold text-gray-900">
              {pathname === '/dashboard/customer' && 'Dashboard Overview'}
              {pathname === '/dashboard/customer/services' && 'My Services'}
              {pathname === '/dashboard/customer/properties' && 'Properties'}
              {pathname === '/dashboard/customer/account' && 'Account Settings'}
              {pathname === '/dashboard/customer/emergency' && 'Emergency Requests'}
              {pathname === '/dashboard/customer/messages' && 'Messages'}
              {pathname === '/dashboard/customer/invoices' && 'Invoices'}
              {pathname === '/dashboard/customer/service-reminders' && 'Service Reminders 👑'}
              {pathname === '/dashboard/customer/tracking' && 'Vehicle Tracking 👑'}
            </h1>
          </div>
